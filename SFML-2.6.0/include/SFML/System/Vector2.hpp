...
public:
    ...
    Vector2<T> toUnit()
    {
        ...
        if (vectorLength == T()) return sf::Vector2<T>(T(), T());

        return  Vector2<T>(x / vectorLength, y / vectorLength);
    }
    ...
    template<typename U>
    friend std::istream& operator>>(std::istream& in, Vector2<U>& vector);
...
template<typename U>
std::istream& operator>>(std::istream& in, Vector2<U>& vector)
{
...
template <typename T>
bool operator ==(const Vector2<T>& left,  T right);
